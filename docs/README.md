# Token-Hop Documentation

이 디렉터리는 **Token-Hop Engine** (크로스 에이전트 프롬프트 컴파일러)의 설계, 아키텍처 및 기술 문서를 포함합니다.

## 목차 (Table of Contents)

### 1. 설계 및 아키텍처 (Design & Architecture)
- [docs/design/token-hop-design-plan.md](file:///home/ubuntu/dev/yunkon-kim/token-hop/docs/design/token-hop-design-plan.md): 
  ...
  - CLI 명세 (`token-hop init/compile/watch/eval/audit`) 및 구현 로드맵

### 2. 유지보수 및 스펙 최신화 (Maintenance & Spec Evolution)
- [docs/maintenance/SPEC_EVOLUTION_PROMPT.md](file:///home/ubuntu/dev/yunkon-kim/token-hop/docs/maintenance/SPEC_EVOLUTION_PROMPT.md):
  - IDE/LLM 도구들의 경로, 확장자, 프론트매터 스키마 변경 시 `token-hop` 엔진을 체계적으로 업데이트하기 위한 **스펙 진화 프롬프트 템플릿 및 6단계 체크리스트**
