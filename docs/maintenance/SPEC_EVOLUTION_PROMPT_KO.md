...
### 4. Register in Engine & CLI (`pkg/engine/` & `cmd/parkour/`)
- Update `pkg/engine/engine.go`: Register the tool name in `ParseSource()` and `EmitTarget()`.
- Update `cmd/parkour/main.go`: Add the tool name to CLI flag descriptions (`--from`, `--to`).
...
- Update `docs/design/agent-parkour-design-plan.md` (Mapping Matrix & Target Specs).
...
- **Cursor AI**: https://docs.cursor.com/context/rules-for-ai
...
2. **GitHub Copilot**:
   ...
   - [GitHub Copilot Prompt Files & Skills](https://docs.github.com/en/copilot)
